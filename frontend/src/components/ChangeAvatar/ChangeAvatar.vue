<template>
  <el-dialog v-model="dialogVisible" title="请选择你要更换的头像" width="30%" :before-close="onCloneAvatar">
    <el-space wrap>
      <el-avatar v-for="(item, index) in avatarList" :key="item" :size="60" :src="item" @click="onClickAvatar(index + 1)"
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const onEditAvatar = () => {
      context.emit("onEditAvatar", { ...props.user, avatar: avatarList.value[Number(isActive.value) - 1] });
    };
